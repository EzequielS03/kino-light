 ...
 * mira ninguna sesión de Kino, solo [MagisAccountState]), la condición pura detrás de "ofrecer vincular
...
class TvOfertaVincularMagisTest {
    ...
    @Test fun `sin Magis vinculado y sin descartar SI se ofrece`() {
        assertEquals(true, debeOfrecerVincularMagis(MagisAccountState.None, descartada = false))
    }

    @Test fun `ya vinculado no se ofrece, aunque descartada sea false`() {
        assertEquals(
            false,
            debeOfrecerVincularMagis(MagisAccountState.Linked("a@b.co"), descartada = false),
        )
    }

    @Test fun `sin vincular pero ya descartada (Ahora no) no se ofrece`() {
        assertEquals(false, debeOfrecerVincularMagis(MagisAccountState.None, descartada = true))
    }

    @Test fun `vinculado Y descartada tampoco se ofrece (caso limite, las dos razones a la vez)`() {
        assertEquals(
            false,
            debeOfrecerVincularMagis(MagisAccountState.Linked("a@b.co"), descartada = true),
        )
    }
}
