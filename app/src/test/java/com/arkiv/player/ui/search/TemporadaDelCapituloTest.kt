...
class TemporadaDelCapituloTest {
     ...
     * [capituloDeCaracol] y de ahí a `DituEntities.buildSerie`. En un grupo, la serie trae UNA
     * temporada (la 1) para todas, así que si el elegido la tomara de ahí, o se buscara por número,
     * el 1 de la T2 reproduciría el 1 de la T1.
     */
    @Test fun `en un grupo tocar el capitulo 1 de la T2 reproduce el de la T2`() {
        ...
        val guardada = DituEntities.buildSerie(
            ...
            capitulos = lista.map { capituloDeCaracol(it, delGrupo) },
            elegido = capituloDeCaracol(tocado, delGrupo),
        ...
        val suelto = DituEntities.build(
            ...
            season = temporadaDelCapitulo(tocado, delGrupo),
        )
