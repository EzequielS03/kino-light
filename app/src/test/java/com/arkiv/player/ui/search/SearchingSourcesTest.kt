...
class SearchingSourcesTest {
    ...
    @Test fun `on starting, everything is searching`() {
        SearchingSources.starting().searching(SourceTab.TODO, SourceTab.MAGIS, SourceTab.CARACOL)
    }
    ...
    @Test fun `magis responded, its tab turns off and caracol and todo keep going`() {
        SearchingSources.starting().sourceFinished("magis").searching(SourceTab.TODO, SourceTab.CARACOL)
    }

    @Test fun `caracol went down, its tab turns off just the same`() {
        SearchingSources.starting().sourceFinished("ditu").searching(SourceTab.TODO, SourceTab.MAGIS)
    }
