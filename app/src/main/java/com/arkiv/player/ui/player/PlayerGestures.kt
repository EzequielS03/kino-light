...
internal class GesturesState(
    ...
    /**
     * Volume 0..100 of whatever's playing. In ExoPlayer it's a 0..1 factor, so it gets converted —
     * and rounded, so raising and lowering one step returns to the same number instead of drifting.
     *
...
internal fun rememberGesturesState(
    ...
    return remember(local, settings) { GesturesState(local, settings) { latest() } }
}
